...
async function runDoctor(options = {}) {
  ...
  function add(name, ok, detail, fatal = true) {
  ...
  const selectedEngine = options.engine || 'codex';
  for (const name of listEngines()) {
    try {
      const status = await getEngine(name).health();
      const isSelected = name === selectedEngine;
      add(
        `Engine: ${name}`,
        !!status.available,
        status.available ? (status.binary || 'available') : status.reason || status.authHint || 'not available',
        isSelected,
      );
      if (isSelected && !status.available) {
        nextSteps.push(`Install or configure the selected engine "${selectedEngine}".`);
      }
    } catch (error) {
      add(`Engine: ${name}`, false, error.message, name === selectedEngine);
    }
  }
  ...
  add(`HTTP port ${httpPort}`, httpFree, httpFree ? 'available' : 'occupied; runtime will use automatic fallback', false);
  add(`WebSocket port ${wsPort}`, wsFree, wsFree ? 'available' : 'occupied; runtime will use automatic fallback', false);

  const ok = checks.every((check) => check.ok || check.fatal === false);
